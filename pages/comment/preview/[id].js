...
const preview = ({ id, previewData }) => {
	return (
		<div>
			<h1>Preview ID: {id}</h1>
			<p>
				This is Preview Data: {previewData?.qId},{' '}
				{previewData?.comments[0]?.comment}
			</p>
			<Link href={`/api/preview?id=${id}`}>Preview</Link>
		</div>
	);
};
...
export const getStaticProps = async ({
	...
	console.log(`/comment/preview/${params.id} context:`, preview, previewData);
	return {
